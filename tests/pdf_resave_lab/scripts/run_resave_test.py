...
from tests.pdf_resave_lab.scripts.resavers import RESAVERS
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run an automatic PDF resave test before split.")
    parser.add_argument("--pdf", help="Optional PDF path. Defaults to the newest PDF in input/.")
    parser.add_argument("--resaver", required=True, help="Resaver key, e.g. mutool_clean")
...
def main():
    ...
    resaver = RESAVERS.get(args.resaver)
    if resaver is None:
        available = ", ".join(sorted(RESAVERS))
        raise ValueError(f"Unknown resaver '{args.resaver}'. Available: {available}")

    run_name = args.output_name or f"{pdf_path.stem}_{args.resaver}"
    run_dir = OUTPUT_DIR / run_name
    prepare_output_dir(run_dir, args.keep_old)

    original_pdf = run_dir / "original.pdf"
    resaved_pdf = run_dir / "resaved.pdf"

    shutil.copy2(pdf_path, original_pdf)
    resaver(str(original_pdf), str(resaved_pdf))

    original_split_dir = run_dir / "original_split"
    resaved_split_dir = run_dir / "resaved_split"

    split_pdf_to_images(str(original_pdf), str(original_split_dir))
    split_pdf_to_images(str(resaved_pdf), str(resaved_split_dir))

    print(f"Source: {pdf_path}")
    print(f"Run: {run_dir}")
    print(f"Original PDF: {original_pdf}")
    print(f"Resaved PDF: {resaved_pdf}")
    print(f"Original PNGs: {original_split_dir}")
    print(f"Resaved PNGs: {resaved_split_dir}")
